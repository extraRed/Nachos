...
Thread::Thread(char* threadName)
{
    ...
    //by LMX
    bool flag=0;
    for(int i=0;i<MaxThreads;i++){      //search for a TIDs[i]==0
        if(TIDs[i]==0){
            TID=i;
            TIDs[i]=1;
            flag=1;
            //printf("new allocated TID: %d\n",i);
            break;
        }
    } 
    if(flag==0){
        printf("To many threads! No TID available!\n");
        TID = -1;
    }
    UID=getuid();           //use the OS's userid
    this->priority=MinPriority/2;   //Initialize the priority to the half
    this->timeSlice=DefaultTimeSlice;   //Initialize the time slice
